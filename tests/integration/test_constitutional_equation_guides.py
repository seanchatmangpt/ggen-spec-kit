"""Constitutional equation test for guides"""
import hashlib
...
def test_cli_module_exists() -> None:
    ...
    cli_file = Path("src/specify_cli/commands/guides.py")
    assert cli_file.exists()


def test_ops_module_exists() -> None:
    """Ops module should exist."""
    ops_file = Path("src/specify_cli/ops/guides.py")
    assert ops_file.exists()


def test_runtime_module_exists() -> None:
    """Runtime module should exist."""
    runtime_file = Path("src/specify_cli/runtime/guides.py")
...
def test_rdf_specification_exists() -> None:
    ...
    assert any(str(r[0]).lower() == "guides" for r in results)
...
def test_cli_module_importable() -> None:
    ...
    from specify_cli.commands import guides
    assert guides.app is not None
...
def test_ops_module_importable() -> None:
    ...
    import sys
    sys.path.insert(0, "src")
    from specify_cli.ops import guides
