...
def test_cli_module_exists() -> None:
    ...
    cli_file = Path("src/specify_cli/commands/terraform.py")
    assert cli_file.exists()


def test_ops_module_exists() -> None:
    """Ops module should exist."""
    ops_file = Path("src/specify_cli/ops/terraform.py")
    assert ops_file.exists()


def test_runtime_module_exists() -> None:
    """Runtime module should exist."""
    runtime_file = Path("src/specify_cli/runtime/terraform.py")
...
def test_rdf_specification_exists() -> None:
    ...
    assert any(str(r[0]).lower() == "terraform" for r in results)
...
def test_code_matches_specification() -> None:
    ...
    SELECT ?name WHERE {
        ?cmd a :Command ;
             :name ?name .
        FILTER(LCASE(STR(?name)) = "terraform")
    }
    """
