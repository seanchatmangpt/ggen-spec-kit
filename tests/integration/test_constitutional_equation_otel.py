"""Constitutional equation test for otel"""
...
def test_rdf_specification_exists() -> None:
    ...
    assert any(str(r[0]).lower() == "otel" for r in results)
...
def test_cli_module_importable() -> None:
    ...
    from specify_cli.commands import otel

    assert otel.app is not None
...
def test_ops_module_importable() -> None:
    ...
    sys.path.insert(0, "src")
    from specify_cli.ops import otel
...
def test_runtime_module_importable() -> None:
    ...
    sys.path.insert(0, "src")
    from specify_cli.runtime import otel
