"""Constitutional equation test for tests"""
import hashlib
...
def test_rdf_specification_exists() -> None:
    ...
    assert any(str(r[0]).lower() == "tests" for r in results)
...
def test_cli_module_importable() -> None:
    ...
    from specify_cli.commands import tests
    assert tests.app is not None
