"""Constitutional equation test for build"""
...
def test_rdf_specification_exists() -> None:
    ...
    assert any(str(r[0]).lower() == "build" for r in results)
...
def test_cli_module_importable() -> None:
    ...
    from specify_cli.commands import build
...
def test_runtime_module_importable() -> None:
    ...
    sys.path.insert(0, "src")
    from specify_cli.runtime import build
